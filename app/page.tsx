...
async function* streamReader(res: Response) {
  // Testing data streaming - This method reads data streaming
...
export default function Home() {
  ...
  const transformImageToCode = async (file: File) => {
    ...
    transformToCode(JSON.stringify({ base64Img }));
  };
        ...
        <section className="max-w-5xl w-full mx-auto p-10">
          {step === STEPS.LOADING && (
            <div role="status" className="flex items-center justify-center">
              <svg
                aria-hidden="true"
                className="w-20 h-20 text-gray-200 animate-spin dark:text-gray-600 fill-white"
                viewBox="0 0 100 101"
                fill="none"
          ...
          {step === STEPS.PREVIEW && (
            <div className="rounded-md flex flex-col gap-4">
              <div
                className="w-full h-full border-4 rounded-md border-gray-500 aspect-video"
                style={{
